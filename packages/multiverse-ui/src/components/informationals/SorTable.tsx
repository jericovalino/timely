...
type SortableRowProps<TData extends FieldValues> = {
  row: TData;
  index: number;
  rowKey: number;
  ...
  selectedPrimaryKeys: GetPotentialPrimaryKeys<TData>[];
  setSelectedPrimaryKeys: React.Dispatch<
    React.SetStateAction<GetPotentialPrimaryKeys<TData>[]>
  >;
};

const SortableRow = <TData extends FieldValues>({
  row,
  index,
  rowKey,
  columns,
  ...
  return (
    <tr
      ...
      key={rowKey}
      className={cn(
        "group hover:bg-interface-hovered focus:bg-interface-active",
        isSelected ? "bg-interface-selected-subtle" : ""
      )}
    >
      <td
        data-cell="Drag:"
        className="w-8 cursor-grab py-mds-10 pl-mds-16 active:cursor-grabbing"
        {...listeners}
      >
        <div className="flex items-center justify-center">
          <RxDragHandleDots2 className="text-icon-subtle opacity-60 transition-opacity group-hover:opacity-100" />
        </div>
      </td>
      {rest.hasSelectableRows && (
        <td data-cell="Select:" className="py-mds-10 pl-mds-16">
          <Checkbox
            value={isSelected!}
            onChange={(checked) => {
              const selectedPrimaryKeysSet = new Set(selectedPrimaryKeys);
              switch (checked) {
                case true:
                  if (rest.primaryKey) {
                    selectedPrimaryKeysSet.add(row[rest.primaryKey]);
                  }
                  break;
                default:
                  if (rest.primaryKey) {
                    selectedPrimaryKeysSet.delete(row[rest.primaryKey]);
                  }
                  break;
              }
          ...
          >
            <div className="flex h-full w-full flex-col justify-center">
              {typeof col.render === "function" ? (
                ...
                <Text size="body" color="default" lineHeight="loose">
                  ...
                  <thead className="sticky top-0 z-[1] ">
                    <tr>
                      <th className="w-8 bg-interface-subtle py-mds-14 pl-mds-16">
                        <div className="flex items-center justify-center">
                          <RxDragHandleDots2 className="text-icon-subtle" />
                        </div>
                      </th>
                      {rest.hasSelectableRows && (
                        <th
                          className={
                            "w-8 bg-interface-subtle py-mds-14 pl-mds-16"
                          }
                        >
                          <Checkbox
                            value={
                  ...
                  {!isLoading && data.length === 0 && (
                    <tr className="text-center">
                      <td
                        className="py-mds-12"
                        colSpan={
                          columns.length + (rest.hasSelectableRows ? 1 : 0) + 1
                        }
                      >
            ...
            {!hideHead && (
              <thead className="sticky top-0 z-[1] ">
                <tr>
                  ...
                  {rest.hasSelectableRows && (
                    <th
                      className={"w-8 bg-interface-subtle py-mds-14 pl-mds-16"}
                    >
                      <Checkbox
                        value={
                          Boolean(data.length) &&
                            ...
                            <div className="flex h-full w-full flex-col justify-center">
                              {typeof col.render === "function" ? (
                                ...
                                <Text
                                  ...
                                  lineHeight="loose"
                                >
                                  {getByStringyfiedNestedAttribute(
                                    row,
                                    col.name
                                  )}
                                </Text>
